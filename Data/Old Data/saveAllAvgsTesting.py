# -*- coding: utf-8 -*-
"""
Created on Thu Aug 17 11:58:18 2023
...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 02 16:11:05 2023
...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 03 09:12:47 2023
...
exp_set = [
       ...
       [['/', device, '/sigins/',ch,'/range'], lockRange], #want as low as possible without clipping data (red Over light on Lock-in box)
       #congfigures demods
       [['/', device, '/demods/',ch,'/order'], order],  #order is set to 8. Order of what?
       [['/', device, '/demods/',ch,'/timeconstant'], tc/3.33], #time constant is set to tc=.01
...
#%%
run = 1
folderName = startTime.strftime("%y-%m-%d ") + "1cm UHNA3 Phase-Matching Bandwidth Characterization"
while os.path.exists(folderName + "/" + str(run) + "/signal.csv"):
    run += 1
folderRunName = folderName + "/" + str(run)

signalData = 1
takeBkrdData = 0 # set to 1 to take background data, set to 0 to display signal - background
...
if not os.path.exists(folderName + "/meta.csv"):
    ...
    meta.write("Date,Label,Sets,Start Time,End Time (hr:min:sec),Pump,Stokes,Probe,Frequency,Signal,Range,Dwell,Bandwidth,Data Rate,Order,Start Frequency,Stop Frequency,Step,Num Avgs,Pump Laser,Probe Laser,Probe Filter,Stokes Filter,Notes\n")
    meta.close()
...
for kk in range(0,NoRealizations):
    ...
    for jj in range(0,N_steps): #need to append not overwrite file for each frequency set 
        ...
        '''-------save dwell-time sampling (sampleR) (!!GBs of data!!)--------#
        if signalData == 1:
            dwellSigDir = folderRunName + "/Runs/Signal/Dwell-time Sampling Run " + str(kk) + "/"
            if not os.path.exists(dwellSigDir):
                os.makedirs(dwellSigDir)
                
            csvfile = dwellSigDir + str(int(dataF[jj])) + ".csv"
            ...
            dwellBgDir = folderRunName + "/Runs/Background/Dwell-time Sampling Run " + str(kk) + "/"
            if not os.path.exists(dwellBgDir):
                os.makedirs(dwellBgDir)
                
            csvfile = dwellBgDir + str(int(dataF[jj])) + ".csv"
            with open(csvfile, "w") as output:
                writer = csv.writer(output, lineterminator='\n')
                for val in sampleR:
                    writer.writerow([val])
...
if signalData == 1:     
    csvfile=folderRunName+"/signal.csv"  #csvfile=folderName +"/file name.csv". creates path and file name. add +str() to add values. Creates a separate file for each loop.
    with open(csvfile, "w") as output:
        writer=csv.writer(output, lineterminator='\n')
        for val in aveDataR-background:
            writer.writerow([val])

    csvfile=folderRunName+"/frequency.csv" #csvfile=folderName +"/file name.csv". creates path and file name. add +str() to add values. Creates a separate file for each loop.
    with open(csvfile, "w") as output:
        writer=csv.writer(output, lineterminator='\n')
        for val in dataF:
            writer.writerow([val]) 
...
if not os.path.exists(folderRunName + "/timestamp.csv"):
    ...
    timestampf.write("Date,Label,Run,Data,Start Time,End Time (hr:min:sec),Pump,Stokes,Probe,Frequency,Signal,Range,Dwell,Bandwidth,Data Rate,Order,Start Frequency, Stop Frequency,Step,Num Avgs,Notes\n")
    timestampf.close()

if takeBkrdData == 1:
    bkrdEndTime = datetime.datetime.now()
    timestampf = open(folderRunName + "/timestamp.csv", "a")
    timestampf.write(bkrdStartTime.strftime("%y-%m-%d,,") + str(run) + ",Background" + bkrdStartTime.strftime(",%H:%M:%S") + bkrdEndTime.strftime(",%H:%M:%S,,,,,,"))
    timestampf.write(str(lockRange) + "," + str(dwell) + "," + str(RBW) + "," + str("{:e}".format(rate)) + "," + str(order) + "," + str("{:e}".format(Fstart)) + "," + str("{:e}".format(Fstop)) + "," + str("{:e}".format(F_step)) + "," + str(NoRealizations) +",\n")
    timestampf.close()

if signalData == 1:
    endTime = datetime.datetime.now()
    timestampf = open(folderRunName + "/timestamp.csv", "a")
    timestampf.write(startTime.strftime("%y-%m-%d,,") + str(run) + ",Signal" + startTime.strftime(",%H:%M:%S") + endTime.strftime(",%H:%M:%S,,,,,"))
    timestampf.write(str(run) + "/frequency.csv," + str(run) + "/signal.csv,")
    timestampf.write(str(lockRange) + "," + str(dwell) + "," + str(RBW) + "," + str("{:e}".format(rate)) + "," + str(order) + "," + str("{:e}".format(Fstart)) + "," + str("{:e}".format(Fstop)) + "," + str("{:e}".format(F_step)) +  "," + str(NoRealizations) +",\n")
    timestampf.close()
    
    meta = open(folderName + "/meta.csv", "a")
    meta.write(startTime.strftime("%y-%m-%d,,") + str(run) + startTime.strftime(",%H:%M:%S") + endTime.strftime(",%H:%M:%S,,,,"))
    meta.write(str(run) + "/frequency.csv," + str(run) + "/signal.csv,")
    meta.write(str(lockRange) + "," + str(dwell) + "," + str(RBW) + "," + str("{:e}".format(rate)) + "," + str(order) + "," + str("{:e}".format(Fstart)) + "," + str("{:e}".format(Fstop)) + "," + str("{:e}".format(F_step)) +  "," + str(NoRealizations) +",,,,,\n")
    meta.close()
